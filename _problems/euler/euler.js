...
const Euler8 = {
  ...
  Find the thirteen adjacent digits in the 1000-digit number that have the greatest product. What is the value of this product?`,
    answer: () => Math.max(...`73167176531330624919225119674426574742355349194934
  96983520312774506326239578318016984801869478851843
  85861560789112949495459501737958331952853208805511
  12540698747158523863050715693290963295227443043557
  66896648950445244523161731856403098711121722383113
...
const Euler11 = {
  ...
  What is the greatest product of four adjacent numbers in the same direction (up, down, left, right, or diagonally) in the 20×20 grid?`,
    answer: () => Math.max(...`08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
    49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00
    81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65
    52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91
    22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80
    ...
    01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48`
        ...
        .map(n => Number.parseInt(n)))
        .map((row, x, grid) => row
        .map((_, y) => x <= row.length - 4 && y <= row.length - 4
        ? [
            [
                grid[x][y],
                grid[x + 1][y + 1],
        ...
        ].concat(x <= row.length - 1 && y <= row.length - 4 && x >= 3
            ? [
                [
                    grid[x][y],
                    grid[x - 1][y + 1],
